...
public class JgListWidget<T extends JgListWidget.JgListKey> extends AbstractWidget {
	...
	public JgListWidget(int x, int y, int w, int h, int itemWidth, int itemHeight, int cols,
		...
		this.itemsToShow = 0; //Math.min(this.cols * this.rows, this.items.size());
	    this.from = 0;
	...
	public void render(GuiGraphics gg, int mx, int my, float partialTicks) {
		...
		for(int i = this.from; i < this.to; i++){
            ...
            items.get(i).render(gg, x, y, mx, my, index, 
            		selected == i);
        }
		
		this.slider.render(gg, mx, my, partialTicks);
		...
		for(int i = this.from; i < this.to; i++){
            ...
            items.get(index).renderHovered(gg, x, y, mx, my, index, 
            		selected == i);
        }
	}
	...
	public boolean mouseDragged(double p_94740_, double p_94741_, int p_94742_, double p_94743_, double p_94744_) {
		...
		this.itemsToShow = Math.min(this.cols * this.rows, items.size());
		int remaining = items.size() - itemsToShow;
		int steps = (int) Math.ceil(remaining / (rows + 0.0f));
		int step = (int) Mth.lerp(slider.getProgress(), 0, steps);
        this.offset = (step * rows);/*(int) Mth.lerp(slider.getProgress(), 0, (items.size() - 1) -
	...
	public static abstract class JgListKey extends AbstractWidget {
		...
		public JgListKey() {
			super(0, 0, 0, 0, Component.translatable(""));
		}
		...
		public abstract void render(GuiGraphics gg, int x, int y, 
