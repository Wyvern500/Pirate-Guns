...
public class JgSelectionList implements NarratableEntry, GuiEventListener, Renderable {
	...
	@Override
	public void render(GuiGraphics guiRender, int p_253973_, int p_254325_, float p_254004_) {
		if (this.visible) {
			Tesselator tesselator = Tesselator.getInstance();
			BufferBuilder bufferbuilder = tesselator.getBuilder();
			// Background
			renderBackground(guiRender, bufferbuilder, tesselator);

			for (int i = this.from; i < this.to; i++) {
				renderKey(i, guiRender, bufferbuilder, tesselator);
			}
	...
	protected void renderBackground(GuiGraphics guiRender, BufferBuilder bufferbuilder, Tesselator tesselator) {
		//guiRender.blit();
	...
	protected void renderKey(int i, GuiGraphics guiRender, BufferBuilder bufferbuilder, Tesselator tesselator) {
		if (keys.size() > 0) {
			float f = 0.5f;
			...
			guiRender.drawString(this.font, keys.get(i).key,
					(int) nx + (30 - (font.width(keys.get(i).key) / 2)),
					(int) ny + 4, 16777215);
		}
	}
	...
	public static class Key {
		...
		public void render(GuiGraphics guiRender) {
			guiRender.drawString(this.font, this.key, cols, colWidth, 16777215);
		}
