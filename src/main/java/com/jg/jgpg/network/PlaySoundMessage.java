...
public class PlaySoundMessage {
	...
	public PlaySoundMessage(SoundEvent sound, double x, double y, double z, float volume, float pitch) {
		this(ForgeRegistries.SOUND_EVENTS.getKey(sound).toString(), x, y, z,
				volume, pitch);
	}
	...
	public static void handle(PlaySoundMessage msg, Supplier<Context> ctx) {
		...
		context.enqueueWork(() -> {
			...
			if (player != null && sound != null) {
				System.out.println(ForgeRegistries.SOUND_EVENTS.getKey(sound).toString());
				player.level().playSound((Player) null, msg.x, msg.y, msg.z, sound, SoundSource.NEUTRAL, 
