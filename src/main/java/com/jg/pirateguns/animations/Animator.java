...
public class Animator {
	...
	public void update() {
		if(this.animation != null && model.canPlayAnimation()){
	            ...
	            for(GunModelPart part : currentRotations.keySet()) {
	            	...
	            	try {
	            	part.getTransform().rot[0] = PGMath.rotLerp(this.prog, this.prevRotations
	            			.get(part)[0], this.currentRotations.get(part)[0]);
	            	part.getTransform().rot[1] = PGMath.rotLerp(this.prog, this.prevRotations
	            			.get(part)[1], this.currentRotations.get(part)[1]);
	            	part.getTransform().rot[2] = PGMath.rotLerp(this.prog, this.prevRotations
	            ...
	            if(this.tick-kf.startTick >= kf.dur){
	                ...
	                /*for(Entry<GunModelPart, float[]> e : prevRotations.entrySet()) {
	                	LogUtils.getLogger().info("PR Key: " + e.getKey() + " Value: " + 
	                			Arrays.toString(e.getValue()));
	                }
	                for(Entry<GunModelPart, float[]> e : currentRotations.entrySet()) {
	...
	public void nextTick() {
        ...
        for(GunModelPart part : currentRotations.keySet()) {
        	...
        	try {
        	part.getTransform().rot[0] = PGMath.rotLerp(this.prog, part1[0], 
        			part2[0]);
        	part.getTransform().rot[1] = PGMath.rotLerp(this.prog, this.prevRotations
        			.get(part)[1], this.currentRotations.get(part)[1]);
        	part.getTransform().rot[2] = PGMath.rotLerp(this.prog, this.prevRotations
