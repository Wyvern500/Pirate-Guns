...
public class ClientEventHandler {
	...
	public static final KeyMapping SWITCH = new KeyMapping("key.jgpg.switch", GLFW.GLFW_KEY_C, PirateGuns.MODID);
	public static final KeyMapping DISPLAY = new KeyMapping("key.jgpg.display", GLFW.GLFW_KEY_KP_ADD, PirateGuns.MODID);
	public static final KeyMapping RELOAD = new KeyMapping("key.jgpg.reload", GLFW.GLFW_KEY_R, PirateGuns.MODID);
	public static final KeyMapping LOOKANIM = new KeyMapping("key.jgpg.look", GLFW.GLFW_KEY_LEFT_ALT, PirateGuns.MODID);
	public static final KeyMapping LEFT = new KeyMapping("key.jgpg.left", GLFW.GLFW_KEY_LEFT, PirateGuns.MODID);
	public static final KeyMapping UP = new KeyMapping("key.jgpg.up", GLFW.GLFW_KEY_UP, PirateGuns.MODID);
	public static final KeyMapping DOWN = new KeyMapping("key.jgpg.down", GLFW.GLFW_KEY_DOWN, PirateGuns.MODID);
	public static final KeyMapping RIGHT = new KeyMapping("key.jgpg.right", GLFW.GLFW_KEY_RIGHT, PirateGuns.MODID);
	public static final KeyMapping N = new KeyMapping("key.jgpg.n", GLFW.GLFW_KEY_N, PirateGuns.MODID);
	public static final KeyMapping M = new KeyMapping("key.jgpg.m", GLFW.GLFW_KEY_M, PirateGuns.MODID);
	public static final KeyMapping H = new KeyMapping("key.jgpg.z", GLFW.GLFW_KEY_H, PirateGuns.MODID);
	public static final KeyMapping J = new KeyMapping("key.jgpg.x", GLFW.GLFW_KEY_J, PirateGuns.MODID);
	public static final KeyMapping MINUS = new KeyMapping("key.jgpg.-", GLFW.GLFW_KEY_MINUS, PirateGuns.MODID);

	public static void setup() {
		EntityRenderers.register(EntityRegistries.CANON.get(), (context) -> new CanonRenderer(context));
	...
	public static void renderOverlayPost(RenderGameOverlayEvent.Post e) {
		 ...
		 * 
		 * if (e.getType() == ElementType.TEXT) { if (stack.getItem() instanceof
		 * ItemGun) { ItemGun gun = ((ItemGun) stack.getItem());
		 * mc.font.draw(e.getMatrixStack(),
		 * gun.getNBTBullets(stack) + "/" + gun.getMaxAmmo(stack),
		 * mc.getWindow().getGuiScaledWidth() / 7,
		 * mc.getWindow().getGuiScaledHeight() / 1.5f, 0xFFFFFF); }
		 * else if (stack.getItem() instanceof ItemMag) { ItemMag mag = ((ItemMag)
		 * stack.getItem()); mc.font.draw(e.getMatrixStack(),
		 * mag.getNBTBullets(stack) + "/" + mag.getMaxAmmo(),
		 * mc.getWindow().getGuiScaledWidth() / 7,
		 * mc.getWindow().getGuiScaledHeight() / 1.5f, 0xFFFFFF); }
		 * } else if (e.getType() == ElementType.ALL) { if (Util.canWork(stack)) {
		 * ItemGun gun = (ItemGun) stack.getItem(); if (!init) return; ClientHandler
		 * handler = handlers.get(getPlayer().getUUID()); if (handler == null) return;
		 * if (ServerUtils.hasScope(stack) &&
		 * handler.getAimingHandler().isFullAimingProgress()) {
		 * 
		 * if (gun.getScopeResLoc() != null && gun.getNullPartsResLoc() != null) {
		 * RenderUtil.renderScopeImage(e.getMatrixStack(), gun.getScopeResLoc(),
		 * gun.getNullPartsResLoc(), 16, 8); } } } }
		 */
	}
