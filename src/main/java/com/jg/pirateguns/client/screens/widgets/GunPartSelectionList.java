...
@OnlyIn(Dist.CLIENT)
public class GunPartSelectionList extends CustomSelectionList<GunPartSelectionList.Entry> {

	private Font font;
	private int width;
	
	public GunPartSelectionList(Minecraft p_94442_, GunModel model, Font font, int width) {
		...
		for (int j = 0; j < i; ++j) {
			...
			if (l >= this.y0 + 12 && k <= this.y1 - 11) {
				...
				if (this.isSelectedItem(j)) {
					int l1 = this.x0 - 164 + this.width / 2 - (k1) / 2;
					int i2 = this.x0 - 164 + this.width / 2 + (k1) / 2;
					RenderSystem.disableTexture();
	...
	@OnlyIn(Dist.CLIENT)
	public static class Entry extends CustomSelectionList.Entry<GunPartSelectionList.Entry> {
		public GunModelPart part;
		private GunPartSelectionList list;
		
		public Entry(GunModelPart part, GunPartSelectionList list) {
			this.part = part;
			this.list = list;
		}
		...
		public void render(PoseStack matrix, int p_93524_, int p_93525_, int p_93526_, int p_93527_, int p_93528_,
			...
			list.font.drawShadow(matrix, part.getName(),
					(float) (((list.width / 2) - 80)
							- (list.font.width(part.getName()) / 2)),
