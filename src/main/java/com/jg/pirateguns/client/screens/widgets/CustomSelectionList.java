...
public abstract class CustomSelectionList<E extends CustomSelectionList.Entry<E>> extends AbstractContainerEventHandler
		implements Widget, NarratableEntry {
	protected final Minecraft minecraft;
	protected final int itemHeight;
	protected final List<E> children = new CustomSelectionList.TrackedList();
	protected int width;
	...
	public CustomSelectionList(Minecraft p_93404_, int p_93405_, int p_93406_, int p_93407_, int p_93408_,
			int p_93409_) {
	...
	public E getEntry(int p_93501_) {
		return this.children().get(p_93501_);
	}

	public int addEntry(E p_93487_) {
	...
	public boolean isSelectedItem(int p_93504_) {
		return Objects.equals(this.getSelected(), this.children().get(p_93504_));
	}

	@Nullable
	public final E getEntryAtPosition(double p_93413_, double p_93414_) {
		...
		return (E) (p_93413_ < (double) this.getScrollbarPosition() && p_93413_ >= (double) k && p_93413_ <= (double) l
				&& j1 >= 0 && i1 >= 0 && j1 < this.getItemCount() ? this.children().get(j1) : null);
	}
	...
	public void render(PoseStack p_93447_, int p_93448_, int p_93449_, float p_93450_) {
		if (this.visible) {
			...
			this.hovered = this.isMouseOver((double) p_93448_, (double) p_93449_)
					? this.getEntryAtPosition((double) p_93448_, (double) p_93449_)
					: null;
			...
			if (this.renderTopAndBottom) {
				...
				int l = -100;
				bufferbuilder.begin(VertexFormat.Mode.QUADS, DefaultVertexFormat.POSITION_TEX_COLOR);
				bufferbuilder.vertex((double) this.x0, (double) this.y0, -100.0D).uv(0.0F, (float) this.y0 / 32.0F)
						.color(64, 64, 64, 255).endVertex();
				bufferbuilder.vertex((double) (this.x0 + this.width), (double) this.y0, -100.0D)
						.uv((float) this.width / 32.0F, (float) this.y0 / 32.0F).color(64, 64, 64, 255).endVertex();
				bufferbuilder.vertex((double) (this.x0 + this.width), 0.0D, -100.0D)
						.uv((float) this.width / 32.0F, 0.0F).color(64, 64, 64, 255).endVertex();
				bufferbuilder.vertex((double) this.x0, 0.0D, -100.0D).uv(0.0F, 0.0F).color(64, 64, 64, 255).endVertex();
				bufferbuilder.vertex((double) this.x0, (double) this.height, -100.0D)
						.uv(0.0F, (float) this.height / 32.0F).color(64, 64, 64, 255).endVertex();
				bufferbuilder.vertex((double) (this.x0 + this.width), (double) this.height, -100.0D)
						.uv((float) this.width / 32.0F, (float) this.height / 32.0F).color(64, 64, 64, 255).endVertex();
				bufferbuilder.vertex((double) (this.x0 + this.width), (double) this.y1, -100.0D)
						.uv((float) this.width / 32.0F, (float) this.y1 / 32.0F).color(64, 64, 64, 255).endVertex();
				bufferbuilder.vertex((double) this.x0, (double) this.y1, -100.0D).uv(0.0F, (float) this.y1 / 32.0F)
						.color(64, 64, 64, 255).endVertex();
				tesselator.end();
	...
	protected void centerScrollOn(E p_93495_) {
		this.setScrollAmount((double) (this.children().indexOf(p_93495_) * this.itemHeight + this.itemHeight / 2
				- (this.y1 - this.y0) / 2));
	}
	...
	protected void updateScrollingState(double p_93482_, double p_93483_, int p_93484_) {
		this.scrolling = p_93484_ == 0 && p_93482_ >= (double) this.getScrollbarPosition()
				&& p_93482_ < (double) (this.getScrollbarPosition() + 6);
	}
		...
		if (this.visible) {
			...
			if (!this.isMouseOver(p_93420_, p_93421_)) {
				...
				if (e != null) {
					...
					this.clickedHeader((int) (p_93420_ - (double) (this.x0 + this.width / 2 - this.getRowWidth() / 2)),
	...
	public boolean keyPressed(int p_93434_, int p_93435_, int p_93436_) {
		if (super.keyPressed(p_93434_, p_93435_, p_93436_)) {
			...
			this.moveSelection(CustomSelectionList.SelectionDirection.DOWN);
			return true;
		} else if (p_93434_ == 265) {
			this.moveSelection(CustomSelectionList.SelectionDirection.UP);
			return true;
		} else {
			return false;
		}
	...
	protected void moveSelection(CustomSelectionList.SelectionDirection p_93463_) {
	...
	protected void moveSelection(CustomSelectionList.SelectionDirection p_93465_, Predicate<E> p_93466_) {
		int i = p_93465_ == CustomSelectionList.SelectionDirection.UP ? -1 : 1;
	...
	public boolean isMouseOver(double p_93479_, double p_93480_) {
		return p_93480_ >= (double) this.y0 && p_93480_ <= (double) this.y1 && p_93479_ >= (double) this.x0
				&& p_93479_ <= (double) this.x1;
	}
	...
	public NarratableEntry.NarrationPriority narrationPriority() {
		if (this.isFocused()) {
			...
			return this.hovered != null ? NarratableEntry.NarrationPriority.HOVERED
	...
	void bindEntryToSelf(CustomSelectionList.Entry<E> p_93506_) {
		p_93506_.list = this;
	}
	...
	protected void narrateListElementPosition(NarrationElementOutput p_168791_, E p_168792_) {
		...
		if (list.size() > 1) {
			...
			if (i != -1) {
				p_168791_.add(NarratedElementType.POSITION,
						new TranslatableComponent("narrator.position.list", i + 1, list.size()));
			}
		}
	...
	@OnlyIn(Dist.CLIENT)
	public abstract static class Entry<E extends CustomSelectionList.Entry<E>> implements GuiEventListener {
		/** @deprecated */
		@Deprecated
		protected CustomSelectionList<E> list;

		public abstract void render(PoseStack p_93523_, int p_93524_, int p_93525_, int p_93526_, int p_93527_,
				int p_93528_, int p_93529_, int p_93530_, boolean p_93531_, float p_93532_);
	...
	class TrackedList extends AbstractList<E> {
		...
		public E set(int p_93559_, E p_93560_) {
			...
			CustomSelectionList.this.bindEntryToSelf(p_93560_);
			return e;
		}

		public void add(int p_93567_, E p_93568_) {
			this.delegate.add(p_93567_, p_93568_);
			CustomSelectionList.this.bindEntryToSelf(p_93568_);
		}
